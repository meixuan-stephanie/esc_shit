...
def staff(request, pk):
    ...
    context = {'staff': staff, 'audits': audits, 'audit_count': audit_count}
...
#        tenant=Tenant.objects.all().filter(name=search)
#    else:
#        tenant=Tenant.objects.all()


 #   context = {'audits': audits,'myFilter': myFilter, 'tenant':tenant}


    tenant=Tenant.objects.all()
    myfilter=TenantFilter(request.Get,queryset=tenant)
    tenant=myfilter.qs
    context={'tenant':tenant,'myfilter':myfilter}
